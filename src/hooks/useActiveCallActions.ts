...
import { dismissCallOverlay, teardownIosBuzzCallNative } from '@/native/android-call-overlay';
...
export const useActiveCallActions = () => {
    ...
    }, [currentUser?.id, currentUser?.user_id, dispatch, globalParticipants, isMuted]);
    ...
    }, [buzzData?.buzz_id, currentUser?.full_name, currentUser?.username, dispatch]);

    const endCall = useCallback(async (options?: { skipMedia?: boolean }) => {
        if (!buzzCode || isEndingCallRef.current) return;

        isEndingCallRef.current = true;

        try {
            const callTitle = String(buzzData?.buzz_code || buzzData?.title || 'Buzz call');
