...
export const useOrgBuzzes = ({ search = '', filter = 'all' }: UseOrgBuzzesOptions = {}) => {
    ...
    const fetchBuzzes = useCallback(async (isInitial: boolean) => {
        if (isInitial) {
            setLoading(true);
        } else {
        ...
        const { buzzes: newBuzzes, pagination, error: fetchError } = await BuzzService.getOrgBuzzes(
            currentPage,
            PAGE_LIMIT,
            { search, filter },
        );
        ...
        if (!fetchError) {
            ...
            const currentPageNum = Number(paginationMeta?.current_page ?? currentPage);
            ...
            if (isInitial) {
                ...
                const existingIds = new Set(buzzesRef.current.map((item) => item.buzz_id));
                const filteredNewList = newBuzzes.filter((item) => !existingIds.has(item.buzz_id));
                updatedList = [...buzzesRef.current, ...filteredNewList];
            }

            setBuzzes(updatedList);
            setTotalCount(Number(paginationMeta?.total_items ?? updatedList.length));
            setError(null);
