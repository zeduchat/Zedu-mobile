    ...
    return (
        ...
        >
            <Drawer.Screen name="Tabs">
                {props => <TabNavigator onChatsFocus={setIsChatsFocused} />}
            </Drawer.Screen>
        </Drawer.Navigator>
    );
}
...
import { AgentStack, AgentStackParamList } from './stacks/agents';
...
import { DirectCallStack, DirectCallStackParamList } from './stacks/direct-calls';
...
export const AppNavigator = ({ currentRoute }: AppNavigatorProps) => {
    ...
    const { isMuted, toggleMic, sendQuickEmoji, endCall } = useActiveCallActions();
    ...
    useEffect(() => {
        ...
        const isDirectCallNotification = (notification: any) => {
            const payload = notification?.additionalData || notification?.rawPayload?.custom?.a || {};
            const bodyPayload = typeof notification?.body === 'string'
                ? (() => {
                    try {
            ...
            return payload?.event === 'direct_call_initiated' || bodyPayload?.event === 'direct_call_initiated';
        };
        ...
        const processIncomingCallAnsweredElsewhere = async (notification: any) => {
            ...
            const isAlreadyInThisCall =
                activeBuzzId === answeredDetails.buzzId && Boolean(hasJoinedRef.current);
        ...
        const processIncomingCallInvite = async (notification: any) => {
            ...
            if (!shouldProcessIncomingCallInvite(parsed, {
        ...
        const onClickListener = async (event: any) => {
            ...
            const answeredNotificationPayload = event?.notification?.additionalData
                || event?.notification?.rawPayload?.custom?.a
                || {};
            const answeredBodyPayload = typeof event?.notification?.body === 'string'
                ? (() => {
                    try {
            ...
            if (extractDirectCallAnsweredElsewhereDetails(
                {
                    ...answeredNotificationPayload,
                    ...(answeredBodyPayload && typeof answeredBodyPayload === 'object' ? answeredBodyPayload : {}),
                },
                userRef.current?.user_id || userRef.current?.id,
            )) {
            ...
            const data = event?.notification?.additionalData || event?.notification?.rawPayload?.custom?.a || {};
        ...
        const onForegroundDisplayListener = async (event: any) => {
            ...
            if (isDirectCallNotification(event?.notification)) {
                event.preventDefault();
                if (Platform.OS === 'ios') {
                    console.warn('[OneSignal] direct call notification received on iOS — check VoIP Push logs separately');
                } else {
                    await processIncomingCallInvite(event?.notification);
                }
                return;
        ...
        if (Platform.OS === 'android') {
            ...
            unsubscribeNativeIncomingCallCancelled = subscribeToAndroidIncomingCallCancelled(
                (buzzId) => {
        ...
        if (Platform.OS === 'ios') {
            ...
            unsubscribeNativeIncomingCallCancelled = subscribeToIosIncomingCallCancelled(
                (buzzId) => {
                    handleIncomingDirectCallCancelled(buzzId, {
                        notify: true,
                        dispatch: dispatchRef.current,
    ...
    useEffect(() => {
        const bootstrapAsync = async () => {
            try {
                ...
                const [token, organisation, current_org, user] = (await retrieveMultipleData(keys)) || [];
    ...
    useEffect(() => {
        ...
        const consumeNativeLaunchCall = async () => {
            const launchEvent = Platform.OS === 'ios'
            ...
            const clearLaunch = Platform.OS === 'ios'
                ? clearIosLaunchIncomingCall
                : clearAndroidLaunchIncomingCall;
            ...
            const shouldProcessInvite = shouldProcessIncomingCallInvite(launchEvent.invite, {
        ...
        const onAppStateChange = (nextState: AppStateStatus) => {
            ...
            if (nextState === 'active' && hasJoinedRef.current && buzzDataRef.current) {
            ...
            getIosLaunchIncomingCall().then((launchEvent) => {
                if (!launchEvent) return;
    ...
    const isCallRoute = currentRoute === 'OngoingDirectCall' || currentRoute === 'CallScreen' || currentRoute === 'ChannelCall';
...
>
                {!state?.token ? (
                    ...
                    <Stack.Group>
                        ...
                        <Stack.Screen name="ForgotPasswordEmail" component={require('@/screens/auth/forgot/ForgotPasswordEmailScreen').default} />
                        <Stack.Screen name="ForgotPasswordCode" component={require('@/screens/auth/forgot/ForgotPasswordCodeScreen').default} />
                        <Stack.Screen name="ForgotPasswordReset" component={require('@/screens/auth/forgot/ForgotPasswordResetScreen').default} />
                    </Stack.Group>
                ) : (
            ...
            {state?.isCallMinimized && state?.minimizedFrom === 'CallScreen' && minimizedBuzzCode ? (
                <AgoraConnection id={minimizedBuzzCode} />
            ) : null}

            {state?.isCallMinimized && state?.minimizedFrom === 'ChannelCall' && minimizedChannelId ? (
                <ChannelConnection id={minimizedChannelId} />
            ) : null}

            {state?.isCallMinimized && state?.minimizedFrom === 'OngoingDirectCall' && minimizedChannelId ? (
