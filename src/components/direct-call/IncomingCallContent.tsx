...
const IncomingCallContent = ({
    ...
    useEffect(() => {
        ...
        const createPulse = (animatedValue: Animated.Value, delay: number) => Animated.loop(
            Animated.sequence([
    ...
    const primaryRingStyle = useMemo(() => ({
        opacity: primaryPulse.interpolate({ inputRange: [0, 1], outputRange: [0.42, 0] }),
        transform: [
            {
                scale: primaryPulse.interpolate({ inputRange: [0, 1], outputRange: [1, 1.75] }),
            },
        ],
    }), [primaryPulse]);

    const secondaryRingStyle = useMemo(() => ({
        opacity: secondaryPulse.interpolate({ inputRange: [0, 1], outputRange: [0.28, 0] }),
        transform: [
            {
                scale: secondaryPulse.interpolate({ inputRange: [0, 1], outputRange: [1, 1.45] }),
            },
        ],
    }), [secondaryPulse]);
        ...
        <SafeAreaView edges={['top', 'bottom']} style={styles.safeArea}>
            ...
            <View style={styles.container}>
                ...
                <View style={styles.topRow}>
                    ...
                    <View style={styles.infoPill}>
                        <Ionicons name="time-outline" size={normalize(13)} color="#E7FFF4" />
                        <AppText style={styles.infoPillText}>{formatCountdown(countdown)}</AppText>
                    </View>
                </View>

                <View style={styles.content}>
                    <AppText style={styles.kicker}>Incoming call</AppText>
                    <AppText variant="bold" style={styles.callerName}>{callerName}</AppText>
                    <AppText style={styles.subText}>waiting for your response</AppText>

                    <View style={styles.avatarStage}>
                        <Animated.View style={[styles.pulseRing, primaryRingStyle]} />
                        <Animated.View style={[styles.pulseRing, styles.pulseRingSecondary, secondaryRingStyle]} />
                    ...
                    {isLoading ? (
                        ...
                        <>
                            <View style={styles.actionRow}>
                                <Pressable style={styles.actionGroup} onPressIn={onActionPressIn} onPress={onDecline}>
                                    <View style={[styles.actionButton, styles.declineButton]}>
                                        <Ionicons name="call" size={normalize(24)} color="#FFFFFF" style={styles.declineIcon} />
                                    </View>
                                    <AppText style={styles.actionLabel}>Decline</AppText>
                                </Pressable>

                                <Pressable style={styles.actionGroup} onPressIn={onActionPressIn} onPress={onAccept}>
                                    <View style={[styles.actionButton, styles.acceptButton]}>
                                        <Ionicons name="call" size={normalize(24)} color="#FFFFFF" />
                                    </View>
