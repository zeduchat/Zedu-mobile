...
const IncomingDirectCallModal = ({ invite, visible, onClose }: Props) => {
    ...
    const callerName = useMemo(() => {
        return invite?.caller?.full_name || invite?.caller?.username || 'Incoming call';
    ...
    const handleAccept = async () => {
        ...
        const buzzCode = response.data.buzz_code || response.data.buzz_data?.buzz_code;
        ...
        dispatch({ type: ACTIONS.BUZZ_PARTICIPANTS, payload: joinResult.data?.participants || [] });
    ...
    return (
        <Modal visible={visible} transparent animationType="fade" onRequestClose={handleDecline}>
            <>
                <RingtonePlayer type="incoming" playing={visible && !isLoading && isRingtonePlaying} />
