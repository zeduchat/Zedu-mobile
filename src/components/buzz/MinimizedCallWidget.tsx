...
export const MinimizedCallWidget = ({
    ...
    const positionRef = useRef({ x: screenWidth - normalize(220), y: normalize(120) });
        ...
        if (!panResponderRef.current) {
            panResponderRef.current = PanResponder.create({
                ...
                onPanResponderRelease: (_evt, { dx, dy }) => {
                    const widgetWidth = Platform.OS === 'android' ? normalize(220) : normalize(200);
                    const widgetHeight = Platform.OS === 'android' ? normalize(120) : normalize(56);

                    const newX = Math.max(0, Math.min(positionRef.current.x + dx, screenWidth - widgetWidth));
                    const newY = Math.max(0, Math.min(positionRef.current.y + dy, screenHeight - widgetHeight));
    ...
    return (
        ...
        >
            <View style={styles.androidHeader} {...panResponderRef.current?.panHandlers}>
                <TouchableOpacity style={styles.androidHeaderTapArea} onPress={onExpand} activeOpacity={0.85}>
