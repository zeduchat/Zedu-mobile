...
const MenuOptions = ({ onParticipantsClick, onScreenShareToggle, isScreenSharing = false }: MenuOptionsProps) => {
    ...
    const isHost = String(buzzData?.host_id) === String(currentUser?.user_id);
    ...
    const handleRecording = async () => {
        ...
        if (!buzzId) {
            ShowNotify('Info', 'Recording is unavailable right now');
            return;
        }

        if (!buzzData?.is_recording) {
            const result = await BuzzService.startRecording(buzzId);
            if (result.success) {
                dispatch({
                ...
                ShowNotify('Success', 'Recording started');
            } else {
                ShowNotify('Error', result.error || 'Failed to start recording');
            }
        } else {
            ...
            if (result.success) {
                ...
                ShowNotify('Error', result.error || 'Failed to stop recording');
            }
        }
    };
        ...
        <View style={menuStyles.container}>
            <View style={menuStyles.card}>
                ...
                <TouchableOpacity style={menuStyles.menuItem} onPress={() => onScreenShareToggle?.(!isScreenSharing)}>
                    ...
                    <MaterialCommunityIcons name="monitor-share" size={22} color={isScreenSharing ? '#4F46E5' : '#111B21'} />
                </TouchableOpacity>
                <View style={menuStyles.divider} />
                ...
                {/* {isHost ? (
                    <>
                        <View style={menuStyles.divider} />
                        <TouchableOpacity style={menuStyles.menuItem} onPress={handleRecording}>
                            <AppText style={menuStyles.menuText}>
                                {buzzData?.is_recording ? 'Stop recording' : 'Start recording'}
                            </AppText>
                            <MaterialCommunityIcons
                                name={buzzData?.is_recording ? 'record-circle' : 'microphone'}
                                size={22}
                                color={buzzData?.is_recording ? '#DC2626' : '#111B21'}
                            />
                        </TouchableOpacity>
                    </>
                ) : null} */}
