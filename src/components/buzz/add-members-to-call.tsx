...
export const AddMembersToCall = ({ onClose, onAddMembers, existingParticipants = [] }: any) => {
    ...
    const existingParticipantIds = useMemo(
        () => new Set((existingParticipants || []).map((item: any) => String(item?.user_id ?? item?.id))),
        ...
        () =>
            (orgMembers || [])
                .map((item: any) => ({
                    ...
                    name: item?.name || item?.full_name || item?.username || 'Unknown User',
                }))
    ...
    const filteredFlatList = useMemo(
        () => inviteableMembers.filter((u: any) => u.name?.toLowerCase().includes(search.toLowerCase())),
        [inviteableMembers, search]
    );

    const groupedSections = useMemo(() => dmGroupUsersAlphabetically(filteredFlatList), [filteredFlatList]);
    ...
    const selectedUserObjects = useMemo(
        () => inviteableMembers.filter((item: any) => selected.includes(String(item.id))),
    ...
    const addMembers = async () => {
        ...
        try {
            ...
            dispatch({ type: ACTIONS.ERROR, payload: 'Failed to invite selected members.' });
        } finally {
            setAddLoading(false);
        }
    };
    ...
    return (
        <View style={{ flex: 1 }}>
            <View style={styles.navHeader}>
                ...
                >
                    {addLoading && <ActivityIndicator size="small" color="#7141F8" style={{ marginRight: 6 }} />}
                    <AppText
                        style={[
            ...
            {groupedSections.length === 0 || groupedSections.every((section: any) => section.data.length === 0) ? (
                ...
                <View style={styles.contentContainer}>
                    <SectionList
                        ...
                        renderSectionHeader={({ section }: any) => <AppText style={styles.sectionHeader}>{section.title}</AppText>}
                                ...
                                >
                                    ...
                                    >
                                        <View style={styles.userInfo}>
                                            ...
                                            <FastImage source={{ uri: item.avatar_url || item.default_avatar_url }} style={styles.avatar} />
                                        ...
                                        <View style={[styles.radioCircle, selected.includes(itemId) && styles.radioActive]}>
                                            {selected.includes(itemId) && <Ionicons name="checkmark" size={16} color="#FFF" />}
                                        </View>
                                    </TouchableOpacity>
                                </View>
                            );
