...
import { FlatList, StyleSheet, View, Dimensions, ActivityIndicator, ScrollView } from 'react-native';
...
export const ParticipantsGrid = ({ participants, currentUser, joinLoading }: ParticipantsGridProps) => {
    ...
    if (list.length === 1) {
        ...
        const isMe = String(userId) === String(currentUser?.user_id ?? currentUser?.id);
            ...
            >
                {joinLoading ? (
                    ...
                    list.map((p: any, index: number) => {
                        ...
                        const isMe = String(userId) === String(currentUser?.user_id ?? currentUser?.id);
                        const displayName = isMe ? 'You' : (p.full_name || p.username || p.name || 'User');
                        const hasvideoTrack = p.videoTrack ?? false;
                        const hasaudioTrack = p.audioTrack ?? false;
                        const hasScreenTrack = p.screenTrack ?? false;
                        const agoraUid = isMe ? 0 : (p.agoraNumericUid ?? 0);
