...
const ThreadMessageItem = ({ item, index, messages, onLongPress, isGroup, inverted = false }: any) => {
    ...
    try {
        if (inverted) {
            ...
            showDateHeader = index === (messages.length - 1) ||
                (messages[index + 1] && moment(messages[index + 1].created_at).format('YYYY-MM-DD') !== moment(item.created_at).format('YYYY-MM-DD'));
        } else {
                ...
                <View key={mediaItem.id} style={styles.complexFileWrapper}>
                    <View style={styles.fileMainRow}>
                        ...
                        <View style={styles.fileInfo}>
                            <AppText numberOfLines={1} style={styles.fileNameText}>{mediaItem.file_name}</AppText>
                        ...
                        <View>
                            <View style={styles.senderRow}>
                                <AppText
                                    ...
                                    variant="bold"
                                    numberOfLines={1}
                                    style={{ color: item.senderColor, flex: 1, minWidth: 0, marginRight: 4 }}
                                >
                                    {truncateUsernameForChannel(item.username)}
                                </AppText>
                                <AppText size={10} style={styles.timeText}>{moment(item.created_at).format("h:mm a")}</AppText>
                            </View>
...
const styles = StyleSheet.create({
    ...
    verticalMediaStack: { flexDirection: 'column', gap: 4, marginBottom: 2, marginTop: 5, backgroundColor: Colors.border, borderRadius: 10 },
    mediaImage: { width: width * 0.75, height: 220, borderRadius: 10, borderWidth: 0.5, borderColor: Colors.border },
    ...
    slackThreadContainer: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 8, marginTop: 6, paddingVertical: 4 },
    participantStack: { flexDirection: 'row', alignItems: 'center', marginRight: 8 },
    threadAvatar: { width: 20, height: 20, borderRadius: 4, borderWidth: 1.5, borderColor: '#FFF' },
