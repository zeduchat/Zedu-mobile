...
import { PatchRequest, PutRequest } from '@/utils/requests';
...
export const AddDescription = forwardRef<any, Props>(({ onClose, channel_id }, ref: any) => {
    ...
    const { orgId, channelCallback, channelDetails } = state;
    const [snapPoints, setSnapPoints] = useState(["35%"])

    useEffect(() => {
        setDescription(channelDetails?.description as string)
    }, [])
    ...
    const handleSave = async () => {
        ...
        const { data, error } = await PatchRequest(`/channels/${channel_id}`, {
