...
const MessageItem = ({ item, index, messages, onLongPress, onMentionUser, editMsgId, onEdit }: any) => {
            ...
            >
                <View style={styles.reactionEmojiRow}>
                    {item.reactions.slice(0, 3).map((r: any, i: number) => (
                        <AppText
                            key={i}
                            style={[styles.reactionEmoji, { zIndex: 10 - i, marginLeft: i === 0 ? 0 : -2 }]}
                        >
                            {r.reaction}
                        </AppText>
                    ))}
        ...
        return (
            <AppText style={styles.messageText}>
                {parts.map((part: string, partIndex: number) => {
                    ...
                    return parseMessageTextWithLinks(part).map((segment, segmentIndex) => {
                    ...
                    <View style={[styles.bubble, !item.sent ? styles.sentBubble : styles.receivedBubble, editMsgId === item.thread_id && onEdit
                        ...
                        {item.replyTo && (
                            <View style={styles.replyContainer}>
                                ...
                                <AppText size={13} numberOfLines={1}>{item.replyTo}</AppText>
                            </View>
                        )}
...
const styles = StyleSheet.create({
    ...
    receivedRow: { alignSelf: 'flex-start', alignItems: 'flex-start', paddingLeft: 10, marginTop: 15 },
    sentRow: { alignSelf: 'flex-end', alignItems: 'flex-end', paddingRight: 10, marginTop: 15 },
    groupAvatarContainer: { marginRight: 8 },
    groupSenderAvatar: { width: 30, height: 30, borderRadius: 15, borderWidth: 1, borderColor: Colors.border, objectFit: "contain" },
    ...
    threadAction: { flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-start', marginTop: 10 },
    reactionBadge: {
        position: 'absolute',
        bottom: -20,
