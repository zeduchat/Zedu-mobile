        ...
        <View style={styles.mockSpreadsheet}>
            {[0, 1, 2, 3].map((row) => (
                <View key={row} style={styles.mockSpreadsheetRow}>
                    <View style={[styles.mockSpreadsheetCell, styles.mockSpreadsheetHeader, { backgroundColor: accent }]} />
                    <View style={[styles.mockSpreadsheetCell, { backgroundColor: '#FFFFFF' }]} />
                    <View style={[styles.mockSpreadsheetCell, { backgroundColor: '#FFFFFF' }]} />
                </View>
            ))}
        </View>
...
function DocumentPreviewMock({ accent, label }: { accent: string; label: string }) {
    return (
        <View style={styles.mockDocument}>
            <View style={[styles.mockDocumentSheet, { borderColor: accent }]}>
                <View style={[styles.mockDocumentBadge, { backgroundColor: accent }]}>
                    <AppText variant="bold" style={styles.mockDocumentBadgeText}>{label}</AppText>
                </View>
...
export function FileAttachmentPreviewPanel({
    ...
    return (
        <View style={[panelStyle, { backgroundColor: theme.previewBackground }]}>
            {theme.kind === 'spreadsheet' && <SpreadsheetPreviewMock accent={theme.previewAccent} />}
            {theme.kind === 'presentation' && <PresentationPreviewMock accent={theme.previewAccent} />}
        ...
        >
            ...
            <View style={styles.cardFooter}>
                <View style={[styles.fileIconBox, { backgroundColor: theme.color }]}>
                    <AppText variant="bold" style={styles.fileExtText}>{theme.label}</AppText>
                </View>
