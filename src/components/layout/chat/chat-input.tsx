...
import {
    ...
    Keyboard, Animated, ActivityIndicator, Platform,
} from 'react-native';
...
const BAR_HEIGHTS: number[] = Array.from({ length: BARS }, (_, i) =>
    4 + Math.abs(Math.sin(i * 0.65 + 1) * 11 + Math.cos(i * 0.38) * 5)
);
...
const ChatInput = ({
    ...
    const navBarBottom = navigationBarBottomPadding(insets.bottom);
    ...
    const keyboardLift =
        isAndroid && keyboardHeight > 0
            ? Math.max(0, keyboardHeight - navBarBottom)
            : 0;
    const bottomInset = isAndroid && keyboardHeight === 0 ? navBarBottom : 0;
    ...
    const handleOpenEmoji = () => {
        Keyboard.dismiss();
        setTimeout(() => { onOpenEmoji(); onFocus(); }, 1);
    };
    ...
    const handleTextChange = (text: string) => {
        ...
        if (lengthDelta > 0) {
            nextCursor = prevCursor >= previousText.length
                ? text.length
        ...
        return (
            <View style={[styles.inputContainer]}>
                ...
                <View style={styles.voiceBar}>
                    <TouchableOpacity style={styles.previewPlayBtn} onPress={handlePreviewPlayPause} activeOpacity={0.8}>
                        <FontAwesome5
                        ...
                        {BAR_HEIGHTS.map((h, i) => (
                            <View
                                ...
                                style={[
                                    ...
                                    {
                                        height: h,
                                        backgroundColor: i / BARS <= progress ? Colors.primary : '#C8CCCE',
                                        borderRadius: h / 2,
                                    },
                                ]}
                            />
                    ...
                    <AppText size={11} style={styles.previewTimer}>{timeLabel}</AppText>
                </View>
        ...
        <View style={[styles.inputContainer]}>
            ...
            {!isVoiceRecording && (
                isEmojiOpen ? (
                    ...
                    <TouchableOpacity style={styles.iconBtn} onPress={() => { Keyboard.dismiss(); onMediaPicker(); }}>
                        <Image source={require('@/assets/icons/plus-black.png')} style={styles.inputIcon} />
                    </TouchableOpacity>
                )
            )}
            ...
            {isVoiceRecording ? (
                <View style={styles.recordingBar}>
                    <View style={styles.timerRow}>
                        ...
                        <AppText style={styles.timerText}>{fmtSec(recordingSeconds)}</AppText>
                    </View>
                ...
                <View style={styles.textInputWrapper}>
                    <TextInput
                        ...
                        onPressIn={() => { if (isEmojiOpen) onCloseEmoji(); }}
                        onFocus={() => { if (!isEmojiOpen) onFocus(); }}
                    />
            ...
            {message.length === 0 ? (
                ...
                <TouchableOpacity style={styles.sendBtn} onPress={() => onSend(message)}>
                    <Image source={require('@/assets/icons/send.png')} style={styles.sendIcon} />
                </TouchableOpacity>
            )}
        </View>
...
const styles = StyleSheet.create({
    ...
    sendIcon: { width: 18, height: 18, objectFit: 'contain', tintColor: Colors.white },
});
