...
function mapDmsToDestinations(dms: any[]): ForwardDestination[] {
    return (dms || []).map((chat) => ({
        type: chat.channel_type === 'group_dm' ? 'group_dm' as const : 'dm' as const,
...
function mergeDestinations(channels: ForwardDestination[], dms: ForwardDestination[]) {
...
export const ForwardMessageModal: React.FC<Props> = ({
    ...
    const [selectedDestination, setSelectedDestination] = useState<ForwardDestination | null>(null);
    ...
    const modalTitle = threadChatType === 'dm' ? 'Forward Private Message' : 'Forward Message';
    ...
    const fetchDestinations = useCallback(async (query: string) => {
        ...
        try {
            ...
            const [channelsResponse, dmsResponse] = await Promise.all([
                GetRequest(`/organisations/${orgId}/user-channels?page=1&limit=50&search=${encodedQuery}`),
                GetRequest(`/organisations/${orgId}/dms?page=1&limit=50&search=${encodedQuery}`),
            ]);

            const channelItems = mapChannelsToDestinations(channelsResponse.data?.data || userChannels || []);
            const dmItems = mapDmsToDestinations(dmsResponse.data?.data || dms || []);
    ...
    const filteredDestinations = useMemo(() => {
        ...
        return destinations.filter((destination) => {
            const haystack = `${destination.label} ${destination.subtitle || ''}`.toLowerCase();
            ...
            >
                <View style={styles.header}>
                    <TouchableOpacity onPress={onClose} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
                        <AppText size={17} style={styles.cancelText}>Cancel</AppText>
                    </TouchableOpacity>
                                        ...
                                        {filteredDestinations.map((destination) => (
                                            ...
                                            >
                                                    ...
                                                    <View style={styles.avatarWrap}>
                                                        {destination.avatarUrl ? (
                                                            ...
                                                            <View style={[styles.avatar, styles.avatarPlaceholder]}>
                                                                <AppText variant="bold" size={14} style={styles.avatarInitial}>
                                                                    {destination.label?.charAt(0)?.toUpperCase() || '?'}
                                                                </AppText>
                                                            </View>
                                                        )}
                                                        <View
                                                            style={[
                                                                styles.statusDot,
                                                                { backgroundColor: destination.online ? Colors.online : Colors.offline },
                                                            ]}
                                                        />
                                                    </View>
                                                )}
                                                ...
                                                <View style={styles.destinationTextWrap}>
                                                    <AppText variant="bold" size={16} style={styles.destinationLabel}>
                                                        {destination.label}
                                                    </AppText>
                                                    {destination.subtitle ? (
                                                        <AppText size={15} style={styles.destinationSubtitle} numberOfLines={1}>
                                                            {destination.subtitle}
                                                        </AppText>
                                                    ) : null}
                                                </View>
                    ...
                    <View style={styles.previewCard}>
                        <View style={styles.previewHeader}>
                            <View style={styles.previewAuthorRow}>
                                {senderAvatar ? (
                                    <Image source={{ uri: senderAvatar }} style={styles.previewAvatar} />
                                ) : (
                                ...
                                <AppText variant="bold" size={15} style={styles.previewAuthorName}>
                                    {senderName}
                                </AppText>
                            </View>
                            ...
                            <Text style={styles.previewMessage}>
                                {previewSegments.map((segment, index) => {
                                    ...
                                    if (segment.type === 'mention') {
                                        return (
                                            <Text key={`mention-${index}`} style={styles.previewMention}>
                                                @{segment.label}
                                            </Text>
                                        );
                                    }
                        ...
                        {!preview.hasText && preview.fileAttachment ? (
                            <View style={styles.previewFileRow}>
                                <View style={[styles.previewFileIcon, { backgroundColor: preview.fileAttachment.color }]}>
                                    <AppText variant="bold" size={11} style={styles.previewFileLabel}>
                                        {preview.fileAttachment.label}
                                    </AppText>
                                </View>
                                <AppText size={15} style={styles.previewFileName} numberOfLines={2}>
