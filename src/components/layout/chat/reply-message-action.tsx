...
export const ReplyMessageAction = forwardRef<AppBottomSheetRef, {
    item: any;
    onClose: () => void;
    handleEdit?: () => void;
    ...
    const handleDelete = async () => {
        ...
        try {
            const { error } = await DeleteRequest(
                `/channels/${item?.channels_id || item?.channel_id}/messages/${messageId}`,
            );
            ...
            if (!error) {
                ...
                ShowNotify('Error', typeof error === 'string' ? error : 'Failed to delete message');
            }
        } catch (error) {
