...
export const ReplyMessageAction = forwardRef<
  AppBottomSheetRef,
  {
    item: any;
    onClose: () => void;
...
>(({ item, onClose, handleEdit, threadChatType = 'dm' }, ref) => {
  ...
  const handleDelete = async () => {
    ...
    if (!messageId) {
      ShowNotify('Error', 'Unable to delete this message');
      return;
    }
    ...
    try {
      ...
      if (!error) {
        ...
        ShowNotify(
          'Error',
          typeof error === 'string' ? error : 'Failed to delete message',
        );
      }
    } catch (error) {
      console.error('Delete failed:', error);
      ShowNotify('Error', 'Failed to delete message');
    } finally {
  ...
  const ActionItem = ({
    ...
    >
      <View
        style={[
          styles.iconWrapper,
          isDestructive && { backgroundColor: '#FFF1F0' },
        ]}
      >
        {isLoading ? (
          ...
          <Image
            source={icon}
            style={[styles.icon, isDestructive && { tintColor: '#FF3B30' }]}
          />
        )}
      </View>
      <AppText
        size={15}
        style={[styles.label, isDestructive && { color: '#FF3B30' }]}
      >
        {label}
      </AppText>
    </TouchableOpacity>
          ...
          <>
            <View style={styles.pinnedHeader}>
              ...
              <View style={styles.textContainer}>
                ...
                <AppText size={13} style={{ color: '#667781', marginTop: 2 }}>
