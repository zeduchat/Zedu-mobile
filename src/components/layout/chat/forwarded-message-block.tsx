...
export const ForwardedMessageBlock: React.FC<Props> = ({ data, onMentionUser }) => {
    const segments = parseMessageHtmlForRender(data.message || '');
    const previewImage = data.media.find((mediaItem) => {
        const type = String(mediaItem?.file_type || mediaItem?.type || '').toLowerCase();
        const mimeType = String(mediaItem?.mime_type || mediaItem?.file_mime_type || '').toLowerCase();
        return ['jpg', 'jpeg', 'png', 'gif', 'webp', 'heic', 'image'].includes(type)
            || mimeType.startsWith('image/');
    });

    const fileMedia = data.media.find((mediaItem) => mediaItem && mediaItem !== previewImage);
        ...
        <View style={styles.container}>
            ...
            <View style={styles.content}>
                <View style={styles.authorRow}>
                    ...
                    <AppText variant="bold" size={15} style={styles.senderName} numberOfLines={1}>
                        {data.senderName}
                    </AppText>
                </View>
                ...
                {fileMedia?.file_name ? (
                    <View style={styles.fileRow}>
                        <View style={styles.fileIcon}>
                            <AppText variant="bold" size={10} style={styles.fileIconText}>FILE</AppText>
                        </View>
