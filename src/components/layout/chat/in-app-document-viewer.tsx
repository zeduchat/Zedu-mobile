    ...
    return (
        <View style={styles.chrome}>
            ...
            <View style={styles.chromeTitleWrap}>
                <AppText variant="bold" size={15} numberOfLines={1}>{displayName}</AppText>
...
export function InAppDocumentViewer({
    ...
    const [previewMode, setPreviewMode] = useState<DocumentPreviewMode>('office-online');
    ...
    const displayName = getDocumentDisplayName(file);
    const theme = getFileTheme(displayName);
    const canPreviewInApp = isInAppPreviewSupported(displayName, getDocumentMimeType(file));

    const content = (
        <View style={[styles.viewerContainer, variant === 'embedded' && { height }]}>
            <ViewerChrome
                file={file}
                onClose={onClose}
            ...
            <View style={styles.webviewWrap}>
                {!canPreviewInApp || failed || !previewUri ? (
                    <View style={styles.fallbackState}>
                        <View style={[styles.fallbackIcon, { backgroundColor: theme.color }]}>
                            <AppText variant="bold" style={styles.fallbackIconText}>{theme.label}</AppText>
                        </View>
                        <AppText variant="bold" size={16} style={styles.fallbackTitle} numberOfLines={2}>
                            {displayName}
                        </AppText>
                        <AppText size={13} style={styles.fallbackSubtitle}>
                            In-app preview is unavailable for this file. Open it with another app on your device.
                        </AppText>
                        <TouchableOpacity
                        ...
                        >
                            {openingExternal ? (
                                ...
                                <AppText variant="bold" size={14} style={styles.fallbackButtonText}>
                                    Open in another app
                                </AppText>
                            )}
                        </TouchableOpacity>
                    ...
                    <>
                        {loading && (
                            <View style={styles.loadingOverlay}>
                                ...
                                <AppText size={13} style={styles.loadingText}>Loading preview…</AppText>
                            </View>
                        )}
                        <WebView
