...
const BACKEND_USERS = [
    { id: '1', name: 'Toyosi Bello', role: 'Product Designer', img: require('@/assets/images/user-1.png') },
    { id: '2', name: 'Mabel', role: 'Product Designer', img: require('@/assets/images/user-2.png') },
    { id: '3', name: 'Midas', role: 'Fullstack Dev', img: require('@/assets/images/user-3.png') },
    { id: '4', name: 'Erasmus', role: 'Product Designer', img: require('@/assets/images/user-3.png') },
    { id: '5', name: 'A1', role: 'Product Designer', img: require('@/assets/images/user-1.png') },
    { id: '6', name: 'A2', role: 'Product Designer', img: require('@/assets/images/user-2.png') },
    { id: '7', name: 'A3', role: 'Product Designer', img: require('@/assets/images/user-1.png') },
    { id: '8', name: 'A4', role: 'Product Designer', img: require('@/assets/images/user-3.png') },
];
...
export const AddMembersToCall = ({ onClose, onAddMembers }: any) => {
    ...
    const selectedUserObjects = useMemo(() =>
        BACKEND_USERS.filter(u => selected.includes(u.id)), [selected]);

    const filteredFlatList = useMemo(() =>
        BACKEND_USERS.filter(u => u.name.toLowerCase().includes(search.toLowerCase())), [search]);

    const groupedSections = useMemo(() =>
        groupUsersAlphabetically(filteredFlatList), [filteredFlatList]);
    ...
    return (
        <View>
            <ScrollView
                showsVerticalScrollIndicator={false} style={styles.mainWrapper}>
                ...
                <View style={styles.navHeader}>
                    ...
                    >
                        ...
                        <FontAwesome5Icon name='plus' size={14} color={Colors.primary} style={{ opacity: selected.length === 0 ? 0.3 : 1 }} />
                    </TouchableOpacity>
                </View>
                        ...
                        <View key={section.title}>
                            ...
                            <View style={styles.groupCard}>
                                {section.data.map((user: any, idx: number) => (
                                    <TouchableOpacity
                                        key={user.id}
                                        style={[styles.userRow, idx !== section.data.length - 1 && styles.rowBorder]}
                                        onPress={() => toggleUser(user.id)}
                                    >
                                        ...
                                        <View style={[styles.radioCircle, selected.includes(user.id) && styles.radioActive]}>
                                            {selected.includes(user.id) && <Ionicons name="checkmark" size={16} color="#FFF" />}
                                        </View>
                                    </TouchableOpacity>
                                ))}
                            </View>
            ...
            {selected.length > 0 && (
                <View style={styles.floatingFooter}>
                    <TouchableOpacity
                        ...
                        activeOpacity={0.9}
                        onPress={() => { onAddMembers(selectedUserObjects); onClose(); }}
                    >
...
const styles = StyleSheet.create({
    ...
    userRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 14 },
