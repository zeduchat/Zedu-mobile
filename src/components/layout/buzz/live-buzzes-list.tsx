...
import { getOrgBuzzLabel, isChannelOrgBuzz, isOrgBuzzActive, getOrgBuzzStatusLabel, getOrgBuzzStatusColor } from '@/utils/org-buzz';
...
export const BuzzListItem = ({ buzz }: BuzzListItemProps) => {
    ...
    const handleJoinBuzz = async () => {
        ...
        try {
            ...
            const errorMessage = joinError instanceof Error ? joinError.message : 'Failed to join buzz';
    ...
    return (
        <View style={styles.buzzRow}>
            <View style={[styles.buzzIconWrap, !isActive && styles.buzzIconWrapEnded]}>
                <Feather name="video" size={18} color={isActive ? Colors.online : Colors.textMuted} />
                {isActive && <View style={styles.liveDot} />}
            </View>

            <View style={styles.buzzInfo}>
                <View style={styles.titleRow}>
                    <AppText variant="bold" size={14} numberOfLines={1} style={styles.titleText}>
                ...
                <AppText size={12} style={styles.buzzMeta}>
                    {buzz.participant_count} participant{buzz.participant_count === 1 ? '' : 's'}
                    {' · '}
            ...
            {isActive ? (
                ...
                <View style={styles.endedPill}>
                    <AppText size={12} style={styles.endedText}>Ended</AppText>
                </View>
            )}
        </View>
    );
