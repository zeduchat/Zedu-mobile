...
export type CallOverlayAction = 'expand' | 'endCall' | 'toggleMic' | 'toggleEmoji';
...
export const isAndroidCallOverlaySupported = Platform.OS === 'android' && Boolean(CallOverlay);

export const isIosCallPiPSupported = Platform.OS === 'ios' && Boolean(CallOverlay);
...
export const prepareIosCallPiP = async (title: string, isMuted: boolean): Promise<boolean> => {
...
export const showAndroidCallOverlay = async (title: string, isMuted: boolean): Promise<boolean> => {
...
export const syncOngoingCallCallKit = (
    ...
    CallOverlay.syncOngoingCallCallKit(buzzCode, title, isActive, alternateBuzzId ?? null);
};
...
export const subscribeToCallOverlayActions = (
    listener: (action: CallOverlayAction, payload?: CallOverlayActionPayload) => void,
): (() => void) => {
    if (!isCallOverlaySupported) {
        return () => undefined;
    }
    ...
    const subscription = DeviceEventEmitter.addListener(CALL_OVERLAY_EVENT, handler);
    return () => subscription.remove();
};
