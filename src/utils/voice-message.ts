...
export type VoiceMedia = Pick<Media, 'id' | 'file_name' | 'file_type' | 'mime_type' | 'file_link'>;

/** Fields used to detect voice messages; id/file_link optional for classification helpers */
export type VoiceMediaFields = Pick<Media, 'file_name' | 'file_type' | 'mime_type'> &
...
export function isVoiceMessageMedia(media?: VoiceMediaFields | null): boolean {
    ...
    const mime = (media.mime_type || (media as any).file_mime_type || '').toLowerCase();
    const type = (media.file_type || '').toLowerCase();
    const name = (media.file_name || '').toLowerCase();
    ...
    if (
        ...
        && (type === 'm4a' || type === 'mp4' || mime === 'video/mp4' || mime === 'audio/mp4')
    ) {
        return true;
    }
...
export async function ensureCachedVoiceAudio(media: VoiceMedia): Promise<string> {
...
export function buildVoiceMediaFromUrl(audioUrl: string, item?: { id?: string }): VoiceMedia {
