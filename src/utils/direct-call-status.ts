...
export const isJoinedJoinStatus = (status?: string) => {
    ...
    return normalized === 'active' || normalized === 'accepted' || normalized === 'accept';
};
...
export const shouldCancelDirectCallOnHangup = (
    ...
    if (receivers.some((participant) => isJoinedJoinStatus(participant?.join_status))) {
        return false;
    }

    return receivers.every((participant) => normalize(participant?.join_status) === 'pending');
};
