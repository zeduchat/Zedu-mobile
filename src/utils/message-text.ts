...
const appendTextSegments = (segments: MessageRenderSegment[], content: string) => {
    if (!content) {
        return;
    }
...
export const parseMessageHtmlForRender = (html: string): MessageRenderSegment[] => {
    if (!html) {
        return [];
    }
    ...
    parts.forEach((part) => {
        ...
        parseMessageTextWithLinks(part).forEach((segment) => {
            if (segment.type === 'link' && segment.url) {
                segments.push({ type: 'link', content: segment.content, url: segment.url });
                return;
            }
...
export const parseMessageTextWithLinks = (text: string): MessageTextSegment[] => {
    if (!text) {
        return [];
    }
...
export const buildMessageShareLink = ({
    ...
    const pathSegment =
        context === 'channel' ? 'channels' : context === 'group_dm' ? 'group-dms' : 'dms';

    if (orgSlug) {
        return `${baseUrl}/${encodeURIComponent(orgSlug)}/${pathSegment}/${channelId}?thread_id=${threadId}`;
    }
