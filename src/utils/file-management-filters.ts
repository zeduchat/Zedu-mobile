...
export const sortFileResults = (files: Media[], sortBy: FileManagementFilters['sortBy']): Media[] => {
  return sortFiles(files, sortBy);
};

export const filterFolders = (folders: Folder[], context: FolderFilterContext): Folder[] => {
  ...
  if (dateFilter !== 'any') {
    result = result.filter((folder) => matchesFolderDateFilter(folder, dateFilter));
  }
  if (searchQuery.trim()) {
