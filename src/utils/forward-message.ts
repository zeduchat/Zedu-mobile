...
const IMAGE_TYPES = new Set(['jpg', 'jpeg', 'png', 'gif', 'webp', 'heic', 'image']);
...
function rememberForwardSnapshot({
    ...
    forwardSnapshotBySourceRef.set(`${destinationChannelId}:${sourceThreadId}`, snapshot);
...
function isImageMedia(mediaItem: any): boolean {
    ...
    const mimeType = String(mediaItem?.mime_type || mediaItem?.file_mime_type || '').toLowerCase();
...
export function getForwardMessagePreview(item: any, currentUser?: any): ForwardMessagePreview {
    if (!item) {
        return {
            senderName: 'Unknown',
            message: '',
    ...
    const senderName =
        ...
        || item.sender_name
        || item.email
        || (isCurrentUser ? (currentUser.full_name || currentUser.username || 'You') : '');
    ...
    const senderAvatar =
        ...
        || (isCurrentUser ? (currentUser.avatar_url || currentUser.default_avatar_url) : undefined);
    ...
    const fileMedia = media.find((mediaItem: any) => mediaItem && !isImageMedia(mediaItem));
...
export function getForwardMessageSendUrl(destination: ForwardDestination): string {
...
export function getForwardMessageCallbackAction(destination: ForwardDestination) {
    return destination.type === 'channel' ? ACTIONS.CHANNEL_CALLBACK : ACTIONS.CALLBACK;
}

function extractForwardNested(item: any) {
    return item?.referenced_thread
...
export function getForwardedMessageFromItem(item: any): ForwardedMessageBlockData | null {
    if (!item) return null;
    ...
    if (nested) {
        ...
        return {
            ...
            senderAvatar: source?.avatar_url || source?.default_avatar_url || source?.sender?.avatar_url,
            ...
            sourceChannelName:
                ...
                || 'conversation',
            sourceIsPrivate: source?.is_private ?? nested?.is_private ?? normalizedItem?.source_channel_is_private,
        ...
        if (source) {
            return {
                ...
                sourceChannelName:
                    ...
                    || normalizedItem?.channel_name
                    || 'conversation',
                sourceIsPrivate: source?.is_private ?? normalizedItem?.source_channel_is_private,
...
export function enrichForwardedMessageIfNeeded(item: any) {
    ...
    return {
        ...
        source_channel_name: normalizedItem.source_channel_name || cached.sourceChannelName,
        source_channel_is_private: normalizedItem.source_channel_is_private ?? cached.sourceIsPrivate,
...
export function buildForwardSourceContext(
    ...
    const channelName =
        item?.channel_name
        || state?.channel?.name
        || (sourceType === 'group_dm' ? 'Group message' : sourceType === 'dm' ? 'Direct message' : 'Channel');
...
export async function sendForwardedMessage({
    ...
    const payload = buildForwardPayload({ optionalNote, media, source, originalItem });
    const snapshot = originalItem ? buildForwardSnapshot(originalItem, source, media) : null;
    const referencedThread = originalItem
        ? buildReferencedThreadObject(originalItem, source, media)
        : undefined;
    const trimmedNote = optionalNote?.trim() || '';
    ...
    const { data, error } = await PostRequest(getForwardMessageSendUrl(destination), payload);
