...
export const getOrgBuzzLabel = (buzz: OrgBuzz): string => {
    ...
    if (buzz.channel_type === 'dm_channel' || buzz.channel_type === 'dm' || buzz.buzz_type === 'direct') {
        return 'Direct Buzz';
    }
...
export const matchesOrgBuzzFilter = (buzz: OrgBuzz, filter: OrgBuzzFilter): boolean => {
    if (filter === 'all') return true;
    if (filter === 'channel') return isChannelOrgBuzz(buzz);
    return isDirectOrgBuzz(buzz);
};

export const matchesOrgBuzzSearch = (buzz: OrgBuzz, search: string): boolean => {
