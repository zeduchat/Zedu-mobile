...
import { dismissIncomingDirectCall, getActiveRouteName } from '@/navigation/root-navigation';
...
const resolvePayload = (raw: any) => {
    ...
    const merged: Record<string, any> = {
        ...
        ...(typeof additionalData === 'object' && additionalData ? additionalData : {}),
    };
...
export const extractDirectCallCancelDetails = (raw: any): DirectCallCancelDetails | null => {
...
export const consumePendingIncomingCallCancelled = (buzzId: string): boolean => {
...
export const handleIncomingDirectCallCancelled = async (
    buzzId: string,
    options?: { callerName?: string; notify?: boolean; dispatch?: Dispatch<Action> },
) => {
    const normalizedBuzzId = String(buzzId || '');
    ...
    if (options?.dispatch) {
        await resetDirectCallSession(options.dispatch, { buzzId: normalizedBuzzId });
    }
