...
import { AndroidIncomingCallAction } from '@/services/android-incoming-call.service';
import { IosIncomingCallAction } from '@/services/ios-incoming-call.service';
import {
...
export const acceptAndJoinDirectCallInvite = async (
    ...
    try {
        ...
        const respondResult = await BuzzService.respondToDirectCall(buzzId, 'accept');
        ...
        if (!buzzData?.buzz_code || !buzzData?.agora_token) {
            return { buzzData: null, error: 'Call accepted but buzz data is incomplete' };
        }
        ...
        context.dispatch({ type: ACTIONS.BUZZ_PARTICIPANTS, payload: participantsWithLocalMediaState });
...
export const handleNativeIncomingCallDecline = async (
    ...
    try {
        const declineResult = await BuzzService.respondToDirectCall(buzzId, 'decline');
...
export const handleNativeIncomingCallTimeout = async (
    ...
    try {
        const timeoutResult = await BuzzService.respondToDirectCall(buzzId, 'timeout');
...
export const parseInviteFromNotification = (notification: any): IncomingBuzzInvite | null => {
...
export const handleNativeIncomingCallAction = async (
    ...
    if (action === 'accept') {
        const { buzzData, error } = await acceptAndJoinDirectCallInvite(invite, context);

        if (error || !buzzData) {
            ShowNotify('Error', error || 'Failed to join call');
            await resetDirectCallSession(context.dispatch, { buzzId: String(invite.buzz_id) });
            if (Platform.OS !== 'ios') {
