...
export const useChannels = (orgId: string | null, search: string = '', limit: number = 50) => {
    ...
    const { userChannels: channels, channelCallback } = state;
    ...
    const fetchChannels = useCallback(async (isInitial: boolean = true, searchQuery: string = search) => {
        ...
        const url = `/organisations/${orgId}/user-channels?page=${currentPage}&limit=${limit}&search=${searchQuery}`;
