...
export type DirectCallResponseAction = 'accept' | 'decline' | 'timeout' | 'cancel';
...
class BuzzService {
    ...
    async createChannelBuzz(channel_id:string): Promise<{ data: BuzzData | null; error: string | null }> {
        try {
            const response = await PostRequest<BuzzResponse>('/buzz/create', {channel_id});
    ...
    async directBuzzCall(channel_id: string): Promise<{ data: BuzzData | null; error: string | null }> {
        try {

            const payload = {
                channel_id
            }
            const response = await PostRequest<BuzzResponse>('/buzz/direct-call', payload);
    ...
    async joinBuzz(buzzCode: string): Promise<{ data: BuzzData | null; error: string | null }> {
        try {
            const response = await PostRequest<BuzzResponse>(`/buzz/${buzzCode}/join`);
    ...
    async getBuzzMetadata(buzzCode: string): Promise<{ data: BuzzData | null; error: string | null }> {
        try {
            const response = await GetRequest<BuzzResponse>(`/buzz/${buzzCode}/metadata`);
    ...
    async leaveBuzz(buzzCode: string): Promise<{ success: boolean; error: string | null }> {
        try {
    ...
    async endBuzz(buzzCode: string): Promise<{ success: boolean; error: string | null }> {
        try {
    ...
    async respondToDirectCall(
        ...
        try {
            ...
            if (!response.data) {
                return { data: null, error: 'Invalid response from direct call action' };
            }
    ...
    async getOrgBuzzes(
        ...
        try {
            ...
            const response = await GetRequest<OrgBuzzListResponse | { data: OrgBuzzListResponse }>(
                `/buzz/org/all?${query}`,
            );
            ...
            if (!isSuccess) {
                const errorMsg = payload.message || 'Failed to fetch organization buzzes';
                return { buzzes: [], pagination: null, error: errorMsg };
            }
    ...
    async startRecording(buzzId: string): Promise<{ success: boolean; error: string | null }> {
        try {
    ...
    async stopRecording(buzzId: string): Promise<{ success: boolean; error: string | null }> {
        try {
    ...
    async getBuzzAgoraToken(
        ...
        try {
            ...
            const tokenData = response.data?.data ?? (response.data as BuzzAgoraTokenResult | undefined);

            if (!tokenData?.token || tokenData.uid === undefined || tokenData.uid === null) {
                const errorMsg =
                    response.data?.message || 'Buzz token response is missing required fields';
                return { data: null, error: errorMsg };
            }
