...
export type IosIncomingCallAction = Extract<NativeIncomingCallAction, 'open' | 'accept' | 'decline' | 'timeout'>;
...
const NativeIncomingCall = NativeModules.IncomingCallIOS as IncomingCallIosModule | undefined;

const isIos = Platform.OS === 'ios';

const normalizeInvite = (invite: Record<string, unknown>): IncomingBuzzInvite | null => {
...
const normalizeEvent = (event: IosIncomingCallEvent | null): IosIncomingCallEvent | null => {
    if (!event?.invite) return null;

    const invite = normalizeInvite(event.invite as unknown as Record<string, unknown>);
...
export const getIosLaunchIncomingCall = async (): Promise<IosIncomingCallEvent | null> => {
...
export const markIosIncomingCallConnected = async (buzzId: string): Promise<void> => {
...
export const resetIosIncomingCallSession = async (buzzId?: string): Promise<void> => {
...
export type IosVoipPushKind = 'direct_call' | 'direct_call_cancel' | 'unrecognized';
...
const normalizeVoipPushLog = (payload: Record<string, unknown>): IosVoipPushReceivedEvent => ({
...
export const getIosRecentVoipPushLogs = async (): Promise<IosVoipPushReceivedEvent[]> => {
    ...
    return logs.map((entry) => normalizeVoipPushLog(entry as Record<string, unknown>));
};
