...
const UseGroupReplyChat = ({ channel_id, thread_id }: Props) => {
    ...
    const getChat = useCallback(async () => {
        const { data, error } = await GetRequest(`/group-dms/thread/${thread_id}/channels/${channel_id}?page=1&limit=50`);
    ...
    const loadMore = useCallback(async () => {
        ...
        try {
            const { data, error } = await GetRequest(
                `/group-dms/thread/channels/${channel_id}?page=${nextPage}&limit=50`
            );
            ...
            if (!error && data?.data && data.data.length > 0) {
                ...
                const currentIds = new Set(state.replyChat.map((m: any) => m.thread_id));

                // Only keep messages from the API that aren't already in our state
                const uniqueNewMessages = freshMessages.filter((m: any) => !currentIds.has(m.thread_id));
...
export default UseGroupReplyChat;