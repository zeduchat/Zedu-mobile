...
import { View, Image, StyleSheet, TextInput, NativeSyntheticEvent, TextInputKeyPressEventData, Platform, Keyboard, TouchableOpacity } from 'react-native';
...
const ForgotPasswordCodeScreen: React.FC = () => {
    ...
    const handleResend = async() => {
        ...
        const {data, error} = await PostRequest("/auth/email-request", {email: email});
        if(!error){
            dispatch({type:ACTIONS.SUCCESS, payload: data.message || "Verification code resent"})
        }
        else{
            dispatch({type:ACTIONS.ERROR, payload: error || "An error occurred while resending code"})
        }
    ...
    const handleConfirm = () => {
        ...
        setLoading(true);
        setTimeout(() => {
            dispatch({type:ACTIONS.AUTH_FLOW, payload: {email: email, code: code.join('')}})
            setLoading(false);
        ...
        <Container>
            ...
            >
                ...
                <AppText size={16} style={styles.subtitle}>
                    ...
                    <AppText variant="bold" style={{ color: Colors.black }}>{maskEmail(email)}</AppText>
                </AppText>
                ...
                >
                    {canResend ? (
                        ...
                        <AppText style={styles.resendText}>
                            Resend Code after <AppText style={styles.resendTime}>{timer}s</AppText>
                        </AppText>
                    )}
                </TouchableOpacity>
            </KeyboardAwareScrollView>
