...
export const AddNewMembersScreen = ({ route, navigation }: any) => {
    ...
    const { orgMembers, callback } = state;
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => (
                            <View style={styles.selectedItem}>
                                <View>
                                    {item.profile_url ? (
                                        <Image source={{ uri: item.profile_url }} style={styles.selectedAvatar} />
                                    ) : (
                                        <View style={[styles.selectedAvatar, { backgroundColor: '#E5E7EB', justifyContent: 'center', alignItems: 'center' }]}>
    ...
    const addMembers = async () => {
        ...
        const { error, data } = await PostRequest("/channels/add-multiple", payload);
