...
const ChannelChatScreen = ({ navigation, route }: any) => {
    ...
    const { channel, media, channelsChat, user, callback, channelDetails, buzzIsMuted, buzzShowVideo } = state
    ...
    const { loadMore, isFetchingMore } = UseChannelChat({ channel_id: channel_id as string });
    ...
    const handleMentionUser = (userId: string) => {
        if (!channelDetails?.participants || !Array.isArray(channelDetails?.participants)) return;
        const found = channelDetails?.participants.find((p: any) => p.user_id === userId);
        if (found) {
    ...
    const handleJoinCall = async () => {
        try {
            ...
            const joinResult = await buzzService.joinBuzz(channelDetails?.active_buzz?.buzz_id as string);
    ...
    const handleSendEditMessage = async (content: string, medias: any[] = []) => {
        ...
        dispatch({
            type: ACTIONS.EDIT_CHANNELS_CHAT, payload: {
    ...
    const handleDetails = () => {
        if (!channel?.access) {
            dispatch({ type: ACTIONS.ERROR, payload: "Please join the channel to continue" })
            return
        }
        navigation.navigate("ChannelStack", { screen: "ChannelDetails", params: { channel_id: channel_id } })
    }
        ...
        <Container color={Colors.topNavigation}>
            ...
            <View style={styles.header}>
                ...
                <TouchableOpacity style={styles.headerInfo} onPress={handleDetails}>
                    <AppText variant="bold" size={15} numberOfLines={1}>#{channel?.name || channelDetails?.name}</AppText>
                    <AppText size={11} style={{ color: Colors.textSecondary }}>{channel?.members_count || channel?.users_count} {(channel?.members_count || channel?.users_count) === 1 ? "Member" : "Members"} </AppText>
                </TouchableOpacity>

                <TouchableOpacity style={{ padding: 5, borderRadius: 5, marginRight: 10 }} onPress={activeBuzz ? handleJoinCall : handleVideoCall}>
                    {callLoading ? <ActivityIndicator /> :
                        <View style={styles.videoIconWrap}>
                            <Feather name='video' size={22} color={activeBuzz ? Colors.online : Colors.textMain} />
            ...
            {channelAccess &&
                <FlatList
                    ...
                    renderItem={({ item, index }) => (
                        <View style={{ backgroundColor: onEdit && editMsgId === item.thread_id ? Colors.lightYellow : "transparent" }}>
                            <MessageItem
                                item={{
                                    ...item,
                                    id: item.thread_id,
            ...
            >
                {channelAccess === false ?
                    <View style={{ paddingHorizontal: 15 }}>
                        <AppText style={styles.joinText}>You are viewing #{channel?.name}. Join to start chatting.</AppText>
                        <TouchableOpacity
                        ...
                        >
                            {joinLoading ? <ActivityIndicator color="#FFF" /> : (
                                <AppText variant="bold" style={{ color: '#FFF' }}>Join Channel</AppText>
                            )}
                        </TouchableOpacity>
                    </View>
                    :
...
const styles = StyleSheet.create({
    ...
    headerIcon: { width: 20, height: 20, resizeMode: 'contain', tintColor: '#54656F' },
    avatarStack: { flexDirection: 'row', alignItems: 'center', marginLeft: 5 },
    stackItem: { width: 35, height: 35, borderRadius: 25, borderWidth: 2, borderColor: Colors.topNavigation },
    stackOver: { marginLeft: -15 },
    countBadge: {
        width: 18, height: 18, borderRadius: 11, backgroundColor: '#262626',
        justifyContent: 'center', alignItems: 'center', marginLeft: -10,
        borderWidth: 1.5, borderColor: Colors.topNavigation
    },
