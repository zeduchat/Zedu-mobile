...
const ChannelDetailsScreen = ({ navigation, route }: any) => {
    ...
    const addNewMembers = () => {
        navigation.navigate('ChannelStack', { screen: 'AddNewMembers', params: { channel_id: channel_id } })
    }
            ...
            <ScrollView showsVerticalScrollIndicator={false} bounces={false}>
                ...
                <View style={styles.groupInfoContainer}>
                    <View style={styles.avatarStack}>
                        {channelDetails?.participants.slice(0, 3).map((item, index: number) => (
                            <Image
                        ...
                        <View style={styles.avatarBadge}>
                            <AppText size={10} style={{ color: 'white' }}>{channelDetails?.participants.length}</AppText>
                        </View>
                    </View>
                    <AppText variant="bold" style={styles.groupName}>
                        {channelDetails?.participants?.slice(0, 8).map(p => p.username).join(', ')}
                    </AppText>
                    <AppText size={13} style={{ color: '#8696A0', marginTop: 4 }}>Channel · {channelDetails?.participants?.length} members</AppText>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                        <>
                            {channelDetails?.description ?
                                <AppText style={{ color: Colors.primary, marginBottom: 5 }}>{channelDetails?.description}</AppText>
                                :
                                <AppText style={{ color: Colors.primary, marginBottom: 5 }}>Add channel description</AppText>
                            }
                        </>

                        <Ionicons name="chevron-forward" size={14} color="#8696A0" />
                    </View>
                    <AppText size={12} style={{ color: '#8696A0' }}>Created {moment(channelDetails?.created_at).format("LL")}</AppText>
                </TouchableOpacity>
                ...
                {channelDetails?.preview_media?.length !== 0 &&
                    <View style={styles.sectionPadding}>
                        <TouchableOpacity style={styles.sectionHeader} onPress={() => navigation.navigate('MediaGalleryScreen', { channel_id })}>
                            <AppText size={13} style={{ color: '#8696A0' }}>Media links, and docs</AppText>
                            <View style={styles.mediaCount}>
                                <AppText size={12} style={{ color: '#8696A0' }}>{channelDetails?.preview_media?.length}</AppText>
                                <Ionicons name="chevron-forward" size={14} color="#8696A0" />
                            </View>
                        </TouchableOpacity>
                        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.mediaScroll}>
                            {channelDetails?.preview_media?.map((item: any, index: number) => {
                ...
                <ActionItem
                    ...
                    label="Notifications"
                    subLabel="All"
                    onPress={() => navigation.navigate('NotificationPreference', { channel_id, channelName: channelDetails?.name || 'Channel' })}
                />
                <ActionItem icon="image-outline" label="Media Visibility" onPress={() => navigation.navigate('MediaGalleryScreen', { channel_id })} />
                ...
                <View style={styles.sectionPadding}>
                    <View style={styles.sectionHeader}>
                        <AppText size={13} style={{ color: '#8696A0' }}>{channelDetails?.participants?.length} members</AppText>
                    ...
                    {channelDetails?.participants
                        ...
                        .map((item) => {
                            ...
                            return (
                                <TouchableOpacity key={item.user_id} style={styles.memberRow} onPress={() => navigation.replace('UserDetails', { participant: item, channel_id })}>
                                    {item.avatar_url || item.avatar_url !== "" || item.default_avatar_url ? (
                                        <Image source={{ uri: item.avatar_url || item.default_avatar_url }} style={styles.memberAvatar} />
                                    ) : (
                                        <Image source={require('@/assets/images/user.png')} style={styles.memberAvatar} />
                                    )}
                ...
                <ActionItem icon="log-out-outline" label="Leave Channel" color="#D32F2F" onPress={() => exitSheetRef.current?.expand()} />
...
const styles = StyleSheet.create({
    ...
    stackItem: { width: 55, height: 55, borderRadius: 50, borderWidth: 2, borderColor: Colors.border },
    ...
    linkIcon: { width: 48, height: 48, borderRadius: 24, backgroundColor: Colors.primary, justifyContent: 'center', alignItems: 'center' },
    adminBadge: {
