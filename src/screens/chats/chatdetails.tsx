...
import MentionUserBottomSheet, { MentionUserBottomSheetRef } from '@/components/layout/chat/mention-user-bottomsheet';
...
const ChatDetailScreen = ({ navigation }: any) => {
    ...
    // Mention State
    const [mentionState, setMentionState] = useState<{ query: string, pos: number } | null>(null);
    const [mentionsMetadata, setMentionsMetadata] = useState<any[]>([]);
    const [onEdit, setOnEdit] = useState(false);
    const [editMsgId, setEditMsgId] = useState<string | null>(null);

    const { state, dispatch } = useDataContext();
    const { dmsChat, user, participant, mentionUser, callback, singleDmsChat, singleParticipant, buzzIsMuted, buzzShowVideo, orgId } = state;
    const route = useRoute<RouteProp<ChatStackParamList, 'ChatDetails'>>();
    const { channel_id, participant_id, fromNotification } = route.params;
    ...
    const handleSendMessage = async (content: string, medias: any[] = []) => {
        if (!content.trim() && medias.length === 0 && state.media.length === 0) return;
        ...
        dispatch({ type: ACTIONS.DMS_CHAT, payload: { newMessage: optimisticMessage } });
        setMessage("");
        setMentionsMetadata([]);
        dispatch({ type: ACTIONS.MEDIA, payload: [] });
    ...
    const handleSendEditMessage = async (content: string, medias: any[] = []) => {
        ...
        dispatch({
            type: ACTIONS.EDIT_DM_CHAT, payload: {
        ...
        await PutRequest(`/dms/thread/${editMsgId}/channels/${channel_id}`, payload);
    ...
    const handleSendFromEditor = (caption: string) => {
        if (pendingMedia) { handleSendMessage(caption, state.media); }
        setIsEditorVisible(false);
        setPendingMedia(null);
    };

    const handleEmojiSelect = (emojiObject: any) => {
        if (!isEmojiOpen) { Keyboard.dismiss(); }
        setMessage(prev => prev + emojiObject.emoji);
    };

    const handleLongPress = (item: ChatItem) => {
        setSelectedMsg(item);
        setTimeout(() => { actionSheetRef.current?.expand(); }, 100)
    }
    ...
    const handleBack = () => {
         Keyboard.dismiss();
        if (mentionUser === true) {
            dispatch({ type: ACTIONS.DMS_CHAT, payload: { data: singleDmsChat, page: 1 } })
        ...
        <Container color={Colors.topNavigation}>
            ...
            <View style={styles.header}>
                ...
                <View style={styles.headerAvatarContainer}>
                    <Image source={{ uri: participant[0]?.avatar_url || participant[0]?.default_avatar_url }} style={styles.headerAvatar} />
                </View>
                <TouchableOpacity style={styles.headerInfo} onPress={() => navigation.navigate('UserDetailScreen', { participant: participant[0], channel_id })}>
                    <AppText variant="bold" size={15}>{participant[0]?.username}</AppText>
                </TouchableOpacity>

                <TouchableOpacity style={{ padding: 5, borderRadius: 5, marginRight: 10 }} onPress={handleVideoCall}>
                    {callLoading ? <ActivityIndicator /> :
                        <Feather name='video' size={22} />
                    }
                </TouchableOpacity>
            </View>

            <Image source={require('@/assets/images/chat-bg.png')} style={StyleSheet.absoluteFill} resizeMode="repeat" />
            ...
            <FlatList
                ...
                renderItem={({ item, index }) => (
                    <View style={{ marginBottom: 15, backgroundColor: onEdit && editMsgId === item.thread_id ? Colors.lightYellow : "transparent" }}>
                        <MessageItem
                            item={{
                                ...item,
                                id: item.thread_id,
            ...
            >
                ...
                <ChatInput
                    ...
                    onSend={(content: any) => onEdit ? handleSendEditMessage(content) : handleSendMessage(content)}
                ...
                {isEmojiOpen && (
                    <View style={styles.emojiWrapper}>
                        <EmojiKeyboard onEmojiSelected={handleEmojiSelect} categoryPosition="bottom" enableSearchBar emojiSize={25} />
                    </View>
                )}
            </KeyboardAvoidingView>

            <MessageAction ref={actionSheetRef} item={selectedMsg} onClose={() => setSelectedMsg(null)} handleEdit={handleEdit} />
            <MediaPickerSheet ref={pickerSheetRef} setPendingMedia={setPendingMedia} setIsEditorVisible={setIsEditorVisible} />
            {pendingMedia && <MediaEditorModal visible={isEditorVisible} media={pendingMedia} onClose={() => { setIsEditorVisible(false); setPendingMedia(null); }} onSend={handleSendFromEditor} />}
            {mentionUserSheetRef && <MentionUserBottomSheet ref={mentionUserSheetRef} />}
        </Container>
    );
};
...
const styles = StyleSheet.create({
    ...
    backBtn: { alignItems: 'center', height: 40, width: 40, paddingHorizontal: 5, justifyContent: 'center' },
    headerIcon: { width: 20, height: 20, objectFit: 'contain', tintColor: '#54656F' },
    headerAvatarContainer: { marginLeft: 5, borderWidth: 1, borderRadius: 18, borderColor: Colors.border },
