...
const ChatThreadScreen = ({ navigation, route }: any) => {
    ...
    const { media, replyChat, selectedMsg: selectedMessage } = state
    const { thread_id, channel_id, chatType = 'dm' } = route.params as {
        thread_id: string;
        channel_id: string;
    ...
    const handleSendMessage = async (content: string, medias: any[] = []) => {
        ...
        await PostRequest(getThreadMessageSendUrl(chatType, channel_id), payload);
        dispatch({
            type: getThreadMessageCallbackAction(chatType),
            payload: chatType === 'channel' ? !state.channelCallback : !state.callback,
        });
        clearUploads();
    };
    ...
    return (
        <Container color={Colors.topNavigation}>
            ...
            {selectedMsg && <ReplyMessageAction ref={actionSheetRef} item={selectedMsg} onClose={onClose} />}
