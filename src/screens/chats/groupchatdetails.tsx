...
const GroupChatDetailScreen = ({ navigation, route }: any) => {
    ...
    const { orgId, dmsChat, user, participant, media, callback, buzzIsMuted, buzzShowVideo } = state
    ...
    const handleSendEditMessage = async (content: string, medias: any[] = []) => {
        ...
        await PutRequest(`/group-dms/thread/${editMsgId}/channels/${channel_id}`, payload);
    ...
    const handleNavigate = () => {
        ...
        navigation.navigate("ChatStack", { screen: "GroupDetailsScreen", params: { channel_id: channel_id } })
    }
    ...
    return (
        <Container color={Colors.topNavigation}>
            ...
            <View style={styles.header}>
                ...
                <TouchableOpacity style={styles.headerInfo} onPress={() => navigation.navigate("ChatStack", { screen: "GroupDetailsScreen", params: { channel_id: channel_id } })}>
                    <AppText variant="bold" size={13} numberOfLines={1}>
                        {participant.map(p => p.username).join(', ')}
                    </AppText>
                    <AppText size={11} style={{ color: Colors.textSecondary }}>tap here for group info</AppText>
                </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    countBadge: {
        ...
        borderWidth: 1.5, borderColor: Colors.topNavigation, zIndex: 9, marginTop: 10
    },
