...
const DmDetailsScreen = ({ navigation, route }: any) => {
    ...
    const directCallee = (groupDetails?.participants || []).find(
        (item: any) => String(item?.user_id || item?.id) !== String(currentUser?.user_id || currentUser?.id)
    );
                ...
                <View style={styles.profileHero}>
                    ...
                    <View style={styles.quickActionRow}>
                        ...
                        >
                            ...
                            <AppText size={12} style={styles.circleActionText}>Video</AppText>
                        </TouchableOpacity>
                    ...
                    <View style={styles.sectionPadding}>
                        ...
                        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.mediaScroll}>
                            {groupDetails?.preview_media?.map((item: any, index: number) => {
                                ...
                                const isVideo = item.mime_type?.includes("video") || item.mime_type === "application/octet-stream";
                                const isDoc = item.mime_type?.includes("pdf") || item.file_type?.toLowerCase() === "pdf";
