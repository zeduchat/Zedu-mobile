...
const MentionsScreen = () => {
    ...
    const renderMentionCard = ({ item }: { item: MentionApiItem }) => {
        ...
        return (
            <TouchableOpacity style={styles.chatItem} onPress={() => handleMentionPress(item)}>
                <View style={styles.avatarWrapper}>
                    {showAvatar ? (
                        <Image
                            source={{ uri: item.sender_avatar_url || item.sender_default_avatar_url }}
                            style={styles.profilePic}
                        />
                    ) 
                    : (
                        <FontAwesome5Icon name={showLock ? "lock" : "hashtag"} size={15} color="#54656F" />
                    )}
                </View>
                <View style={styles.chatInfo}>
                    ...
                    <View style={styles.chatFooterRow}>
                        <AppText size={14} numberOfLines={1} style={styles.chatMsg}>
                            {item.previe_message ? item.previe_message.replace(/<[^>]*>?/gm, '') : ''}
                        </AppText>
