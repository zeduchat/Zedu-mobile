...
const OngoingDirectCallScreen = ({ route, navigation }: Props) => {
    ...
    const handleMinimize = () => {
        ...
        dispatch({ type: ACTIONS.CALL_MINIMIZED_FROM, payload: 'OngoingDirectCall' });
        navigation.goBack();
    };
    ...
    const {
        ...
        setEmojiTray,
        joinLoading,
    } = useCallScreen({ buzzCode, buzzData, disableEffect: isMinimized, isDirectCall: true });

    const handleAddMembersToCall = async (members: any[]) => {
        if (!members?.length) return;

        const channelId = String(buzzData?.channel_id || state?.buzzData?.channel_id || '');
