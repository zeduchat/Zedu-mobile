...
// -------------------- Auth & Global -----------------------------
interface TokenAction { type: ACTIONS.TOKEN; payload: string | null }
interface UserAction { type: ACTIONS.USER; payload: any | null }
interface SuccessAction { type: ACTIONS.SUCCESS; payload: string | null }
interface ErrorAction { type: ACTIONS.ERROR; payload: string | null }
interface CallbackAction { type: ACTIONS.CALLBACK; payload: boolean }
interface UploadingAction { type: ACTIONS.UPLOADING; payload: boolean }
interface AuthFlowAction { type: ACTIONS.AUTH_FLOW; payload: AuthFlow }

// -------------------- Organisation ------------------------------
interface OrgAction { type: ACTIONS.ORG; payload: any[] }
interface OrgIdAction { type: ACTIONS.ORG_ID; payload: string | null }
interface OrgDataAction { type: ACTIONS.ORG_DATA; payload: any | null }
interface OrgCallbackAction { type: ACTIONS.ORG_CALLBACK; payload: boolean }
...
// ------------------ Chats (DMs) ----------------------------------
interface DmsAction { type: ACTIONS.DMS; payload: Chat[] }
interface DmLoadingAction { type: ACTIONS.DM_LOADING; payload: boolean }
interface ParticipantAction { type: ACTIONS.PARTICIPANT; payload: Participant[] }
interface SingleParticipantAction { type: ACTIONS.SINGLE_PARTICIPANT; payload: Participant[] }
interface ReceiverAction { type: ACTIONS.RECEIVER; payload: object | null }
interface GroupDetailsAction { type: ACTIONS.GROUP_DETAILS; payload: Group }
interface MediaAction { type: ACTIONS.MEDIA; payload: any[] }
interface GroupCallbackAction { type: ACTIONS.GROUP_CALLBACK; payload: boolean }
interface SelectedMsgAction { type: ACTIONS.SELECTED_MSG; payload: any | null }
...
// -------------------- Channels ----------------------------
interface UserChannelsAction { type: ACTIONS.USER_CHANNELS; payload: [] }
interface AllChannelsAction { type: ACTIONS.ALL_CHANNELS; payload: [] }
interface ChannelLoadingAction { type: ACTIONS.CHANNEL_LOADING; payload: boolean }
interface ChannelDetailsAction { type: ACTIONS.CHANNEL_DETAILS; payload: Channel }
interface ChannelAction { type: ACTIONS.CHANNEL; payload: Channel }
interface ChannelCallbackAction { type: ACTIONS.CHANNEL_CALLBACK; payload: boolean }
...
interface ReplyCallbackAction { type: ACTIONS.REPLY_CALLBACK; payload: boolean }


// -- Agents --

interface AgentsAction { type: ACTIONS.AGENTS; payload: Agent[] }
interface AgentAction { type: ACTIONS.AGENT; payload: Agent | null }
interface AgentCallbackAction { type: ACTIONS.AGENT_CALLBACK; payload: boolean }
...
// -----Buzz ------------
interface BuzzDataAction { type: ACTIONS.BUZZ_DATA; payload: any }
interface BuzzParticipantsAction { type: ACTIONS.BUZZ_PARTICIPANTS; payload: any[] }
...
interface BuzzChatsAction { type: ACTIONS.BUZZ_CHATS; payload: any[] }
interface FloatingEmojisAction { type: ACTIONS.FLOATING_EMOJIS; payload: any[] }
interface AddFloatingEmojiAction { type: ACTIONS.ADD_FLOATING_EMOJI; payload: any }
interface RemoveFloatingEmojiAction { type: ACTIONS.REMOVE_FLOATING_EMOJI; payload: string | number }
interface HasJoinedAction { type: ACTIONS.HAS_JOINED; payload: boolean }
interface BuzzIsMutedAction { type: ACTIONS.BUZZ_IS_MUTED; payload: boolean }
interface BuzzShowVideoAction { type: ACTIONS.BUZZ_SHOW_VIDEO; payload: boolean }
interface BuzzIsScreenSharingAction { type: ACTIONS.BUZZ_IS_SCREEN_SHARING; payload: boolean }
interface BuzzJoinLoadingAction { type: ACTIONS.BUZZ_JOIN_LOADING; payload: boolean }
interface CallMinimizedAction { type: ACTIONS.CALL_MINIMIZED; payload: boolean }
interface CallMinimizedFromAction { type: ACTIONS.CALL_MINIMIZED_FROM; payload: 'OngoingDirectCall' | 'CallScreen' | 'ChannelCall' | null }
interface ResetDirectCallSessionAction { type: ACTIONS.RESET_DIRECT_CALL_SESSION }

// --- Settings ---
interface StatusCallbackAction { type: ACTIONS.STATUS_CALLBACK; payload: boolean }
interface BillingsAction { type: ACTIONS.BILLINGS; payload: any[] }


// --- Mentions ---
interface MentionsListAction { type: ACTIONS.MENTIONS_LIST; payload: ThreadListItem[] }
interface UnseenThreadCountAction { type: ACTIONS.UNSEEN_THREAD_COUNT; payload: number }
interface LoadThreadCallbackAction { type: ACTIONS.LOAD_THREAD; payload: boolean }
interface MentionUserAction { type: ACTIONS.MENTION_USER; payload: boolean }
