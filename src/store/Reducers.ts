...
const reducers = (state: AppState, action: Action): AppState => {
    ...
    switch (action.type) {
        ...
        case ACTIONS.ORG_MEMBERS:
            return {
                ...state,
                orgMembers: action.payload.page === 1
                    ? (action.payload.data || [])
        ...
        case ACTIONS.DMS_CHAT:
            if (action.payload.newMessage) {
                ...
                if (!newMessage.isOptimistic) {
                    ...
                    const cleanList = state.dmsChat.filter(msg =>
                        !msg.isOptimistic && msg.thread_id !== newMessage.thread_id
                    );
            ...
            return {
                ...state,
                dmsChat: action.payload.page === 1
                    ? (action.payload.data || [])
                    : [
                        ...state.dmsChat,
                        ...(action.payload.data || []).filter(
                            (newItem: any) => !state.dmsChat.some((oldItem: any) => oldItem.thread_id === newItem.thread_id)
                        )
                    ]
            };
        ...
        case ACTIONS.SINGLE_DMS_CHAT:
            if (action.payload.newMessage) {
                ...
                if (!newMessage.isOptimistic) {
                    const cleanList = state.singleDmsChat.filter(msg =>
                        !msg.isOptimistic && msg.thread_id !== newMessage.thread_id
                    );
            ...
            return {
                ...state,
                singleDmsChat: action.payload.page === 1
                    ? (action.payload.data || [])
                    : [
                        ...state.singleDmsChat,
                        ...(action.payload.data || []).filter(
                            (newItem: any) => !state.singleDmsChat.some((oldItem: any) => oldItem.thread_id === newItem.thread_id)
                        )
                    ]
            };
        ...
        case ACTIONS.CHANNELS_CHAT:
            if (action.payload.newMessage) {
                const newMessage = enrichForwardedMessageIfNeeded(action.payload.newMessage);

                // 1. If we receive a REAL message (from Centrifugo)
                if (!newMessage.isOptimistic) {
                    // Filter out the optimistic placeholder AND any existing duplicate thread_id
                    const cleanList = state.channelsChat.filter(msg =>
                        !msg.isOptimistic && msg.thread_id !== newMessage.thread_id
                    );
            ...
            return {
                ...state,
                channelsChat: action.payload.page === 1
                    ...
                    : [
                        ...
                        ...(action.payload.data || [])
                            ...
                            .filter(
                            (newItem: any) => !state.channelsChat.some((oldItem: any) => oldItem.thread_id === newItem.thread_id)
                        )
                    ]
            };
        ...
        case ACTIONS.REPLY_CHAT:
            ...
            return {
                ...state,
                replyChat: action.payload.page === 1
                    ? (action.payload.data || [])
                    : [
                        ...state.replyChat,
                        ...(action.payload.data || []).filter(
                            (newItem: any) => !state.replyChat.some((oldItem: any) => {
        ...
        case ACTIONS.AGENTS_CHAT:
            if (action.payload.newMessage) {
                ...
                if (!newMessage.isOptimistic) {
                    // Filter out the optimistic placeholder AND any existing duplicate thread_id
                    const cleanList = state.agentsChat.filter(msg =>
                        !msg.isOptimistic && msg.thread_id !== newMessage.thread_id
                    );
            ...
            return {
                ...state,
                agentsChat: action.payload.page === 1
                    ? (action.payload.data || [])
                    : [
                        ...state.agentsChat,
                        ...(action.payload.data || []).filter(
                            (newItem: any) => !state.agentsChat.some((oldItem: any) => oldItem.thread_id === newItem.thread_id)
                        )
                    ]
            };
            ...
            if (notification_type === "buzz_started") {
                ...
                return {
                    ...
                    userChannels: state.userChannels.map((channel: any) => {
                        if (String(channel.channels_id) === String(buzzEventData.channel_id)) {
                            return {
                                ...channel,
                                active_buzz: {
                                    ...
                                    participant_count: buzzEventData?.participant_ids?.length || 1,
        ...
        case ACTIONS.BUZZ_PARTICIPANT_MEDIA_PATCH:
            return {
                ...state,
                buzzParticipants: (state.buzzParticipants || []).map((participant: any) => {
                    const byUserId = action.payload.user_id
                        ? String(participant?.user_id || participant?.id) === String(action.payload.user_id)
                        : false;
                    const byAgoraUid = action.payload.agoraNumericUid != null
                        ? Number(participant?.agoraNumericUid) === Number(action.payload.agoraNumericUid)
                        : false;
                    ...
                    return {
                        ...participant,
                        ...(action.payload.agoraNumericUid != null ? { agoraNumericUid: action.payload.agoraNumericUid } : {}),
                        ...(typeof action.payload.audioTrack === 'boolean' ? { audioTrack: action.payload.audioTrack } : {}),
                        ...(typeof action.payload.videoTrack === 'boolean' ? { videoTrack: action.payload.videoTrack } : {}),
                        ...(typeof action.payload.screenTrack === 'boolean' ? { screenTrack: action.payload.screenTrack } : {}),
                    };
                }),
            };
        case ACTIONS.BUZZ_PARTICIPANT_REMOVE:
            return {
                ...state,
                buzzParticipants: (state.buzzParticipants || []).filter((participant: any) => {
                    const byUserId = action.payload.user_id
                        ? String(participant?.user_id || participant?.id) === String(action.payload.user_id)
                        : false;
                    const byAgoraUid = action.payload.agoraNumericUid != null
        ...
        case ACTIONS.REMOVE_FLOATING_EMOJI:
            return {
                ...state,
                floatingEmojis: (state.floatingEmojis || []).filter((e: any) => e.id !== action.payload)
            };
